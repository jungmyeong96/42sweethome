...
/*   Updated: 2021/01/28 20:51:23 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_check_precision(va_list ap, char *spec, t_info *info)
{
	...
	if (*tmp == '.')
	{
		...
		if (*tmp && (!('0'<= *tmp && *tmp <= '9') && (*tmp) != '*'))//only .
			info->p_existence = 2;
		if ((!(pre_num = ft_num_asterisk(ap, &tmp, &error))) && error == 1)//when error is occured to spec whether 0 or not
			return (0);
		if (info->p_existence != 2) //. & somenum
			info->p_existence = 1;
	}
