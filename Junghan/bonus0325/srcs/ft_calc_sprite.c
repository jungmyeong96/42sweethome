...
/*   Updated: 2021/03/25 04:54:44 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

void	set_sprite_draw_p(t_sprite *sprite, t_info *info, int tex)
{
	...
	put_in_spritebuf(sprite, info, tex);
}

void	sp_animate(t_sprite *sprite)
{
	if (sprite->vmove <= -30.0)
		sprite->is_animate = 1;
	else if (sprite->vmove >= 60.0)
		sprite->is_animate = 0;
...
void	set_sprite_info(t_sprite *sprite, t_info *info, int i)
{
	...
	set_sprite_draw_p(sprite, info, info->sprite_arr[sprite->sprite_ord[i]].tex);
}
