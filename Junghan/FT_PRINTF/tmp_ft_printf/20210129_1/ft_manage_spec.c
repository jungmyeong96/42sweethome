...
/*   Updated: 2021/01/29 15:15:14 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
const char	*ft_manage_spec(va_list ap, const char *spec, t_info info)
{
	...
	error_check = 0;
	if (*spec == '%')
		error_check = ft_apply_to_percent(ap, spec,info);
	else if (*spec == 'c')
		...
		error_check = ft_apply_to_int(ap, spec, info);
	else if (*spec == 'u')
		error_check = ft_apply_to_unint(ap, spec, info);
