...
/*   Updated: 2021/03/16 19:05:58 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ceiling_shadow(t_wall *wall, t_info *info, int x, int y)
{
	if (y > info->win_hei / 2 * 0.92 + info->pos_z * 0.08 + info->pitch
		...
		info->buf[y][x] = (((((info->texture[5][TEXWID * wall->floor_yt +
		wall->floor_xt]) & 8355711) >> 1) & 8355711) >> 1) & 8355711;
	else if (y > info->win_hei / 2 * 0.95 + info->pos_z * 0.08 + info->pitch
			&& y <= info->win_hei / 2 * 1.02 + info->pos_z * 0.08 + info->pitch)
		info->buf[y][x] = 0;
}

void	floor_shadow(t_wall *wall, t_info *info, int x, int y)
{
	if (y > info->win_hei / 2 * 0.98 + info->pos_z * 0.08 + info->pitch &&
