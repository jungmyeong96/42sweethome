...
/*   Updated: 2021/03/17 01:03:34 by junghan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		**alloc_arr(t_info *info)
{
	...
	if (!(info->minimap = (int **)ft_calloc(info->win_hei / 6, sizeof(int *))))
		error_exit("some error in malloc");
	i = 0;
	while (i < info->win_hei / 6)
	{
		if (!(info->minimap[i] = (int *)ft_calloc(info->win_wid / 3, sizeof(int))))
			error_exit("some error in mallc");
		i++;
	}
	return (0);
