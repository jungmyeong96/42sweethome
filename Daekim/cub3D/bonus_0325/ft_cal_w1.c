...
/*   Updated: 2021/03/13 04:10:58 by daekim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		cal_wall(t_info *p, t_cal *c)
{
	...
	c->draw_st = -(c->line_h) / 2 + p->data.win_h / 2 + p->pitch
		+ (p->pos_z / c->wall);
	if (c->draw_st < 0)
		c->draw_st = 0;
	c->draw_en = c->line_h / 2 + p->data.win_h / 2 + p->pitch
		+ (p->pos_z / c->wall);
